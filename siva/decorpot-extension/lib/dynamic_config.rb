module DecorpotExtension
	def self.dynamic_configuration
		...
		if @selection.length != 0 and @selection.length == 1
			...
			html_path = File.join(WEBDIALOG_PATH, 'new.html')
			...
			UI.messagebox "Please select any one model!", MB_OK
		end

	end
	toolbar = UI::Toolbar.new "Dynamic Configuration"
	cmd = UI::Command.new("Dynamic Configuration") {self.dynamic_configuration}
	cmd.small_icon = "icons/images.png"
	cmd.large_icon = "icons/images.png"
	cmd.tooltip = "Dynamic Toolbars"
	cmd.status_bar_text = "Change Dynamic Configuration"
	cmd.menu_text = "Change Dynamic Configuration"
